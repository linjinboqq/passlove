...
//bylinjinbo
     ...
     * @return
     */
    @Select("select * from dynamics where losttype=#{losttype} and publishtime like #{yesterday} limit #{start},#{end}")
    @Options(useCache = false)
    List<Dynamics> getDynamicsByYesterday(@Param("losttype") int losttype, @Param("start") int start,
                                          @Param("end") int end, @Param("yesterday") String yesterday) throws Exception;

//查询publishtime <昨天以前的的动态
// select * from dynamics where losttype=1 and publishtime >20191113221021 limit 0,15
    @Select("select * from dynamics where losttype=#{losttype} and publishtime < #{yesterday} limit #{start},#{end}")

    @Options(useCache = false)
    List<Dynamics> getDynamicsByYesterday2(@Param("losttype") int losttype, @Param("start") int start,
                                          @Param("end") int end, @Param("yesterday") String yesterday) throws Exception;
//    List<Dynamics> getDynamicsByYesterday(int losttype, int start, int end, String yesterday);
